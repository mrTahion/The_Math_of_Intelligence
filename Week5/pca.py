...
# data prep for PCA
PCA_data = (X_train - np.mean(X_train, axis=0)) / np.std(X_train, axis=0)

# Getting covariance matrix
cov = np.matmul(PCA_data.transpose(), PCA_data) / (PCA_data.shape[0] - 1)
# Get eigen values and axis
eig, axes = np.linalg.eig(cov)

# calculete components for 2d - 2 largest eigen values
pc2d = np.matmul(PCA_data, axes[:, 0:2])
